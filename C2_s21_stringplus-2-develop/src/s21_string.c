...
void d_func(long long int int_num, char *str, int *str_ind, int flag,
...
void f_func(long double float_num, char *str, int *str_ind, int flag,
            int plus_sign, int zero, int length, int left_aligment,
...
void g_func(long double float_num, char *str, int *str_ind, int flag,
            int plus_sign, int zero, int length, int left_aligment,
            int f_accuracy, int sharp) {
  char float_string_part[4096], float_string_full[4096] = "";
  long double int_part, float_part;
...
void c_func(int ch, char *str, int *str_ind, int length, int left_aligment) {
  if (length <= 1) {
    str[*str_ind] = ch;
    *str_ind = *str_ind + 1;
  } else {
...
void s_func(char *str_arg, char *str, int *str_ind, int s_accuracy, int length,
            int left_aligment) {
  int j, len;
...
void sprintf_flags_acception(const char *format, int *left_aligment, int *sharp,
                             int *plus_sign, int *zero, int *i) {
  while (1) {
    if (format[*i] == '-') {
      *left_aligment = 1;
    } else if (format[*i] == '#') {
...
void sprintf_width_acception(const char *format, va_list arg_list,
...
void sprintf_precision_acception(const char *format, va_list arg_list,
...
void sprintf_length_acception(const char *format, int *long_flag, int *i) {
  if (format[*i] == 'l') {
    *long_flag = 'l';
    *i = *i + 1;
  } else if (format[*i] == 'h') {
...
int s21_sprintf(char *str, const char *format, ...) {
  ...
  for (int i = 0; format[i] != '\0'; i++) {
    ...
    if (format[i] == '%') {
      ...
      sprintf_flags_acception(format, &left_aligment, &sharp, &plus_sign, &zero,
                              &i);

      sprintf_width_acception(format, arg_list, &left_aligment, &length, &i);

      sprintf_precision_acception(format, arg_list, &accuracy, &f_accuracy,
                                  &s_accuracy, &i);

      sprintf_length_acception(format, &long_flag, &i);
      ...
      switch (format[i]) {
        case '%':
          ...
          break;
        case 'c':
          c_func(va_arg(arg_list, int), str, &str_ind, length, left_aligment);
          break;
        case 's':
          s_func(va_arg(arg_list, char *), str, &str_ind, s_accuracy, length,
                 left_aligment);
          break;
        case 'd':
        case 'i':
          flag = 'd';
          d_func(va_arg(arg_list, long long int), str, &str_ind, flag,
                 plus_sign, zero, length, left_aligment, accuracy, sharp,
                 long_flag);
          break;
        case 'f':
          set_float_length(&float_num, arg_list, long_flag);
          f_func(float_num, str, &str_ind, flag, plus_sign, zero, length,
                 left_aligment, f_accuracy, sharp);
          break;
        case 'g':
          flag = 'g';
          set_float_length(&float_num, arg_list, long_flag);
          g_func(float_num, str, &str_ind, flag, plus_sign, zero, length,
                 left_aligment, f_accuracy, sharp);
          break;
        case 'G':
          flag = 'G';
